...
const SUBJECTS = [
  ...
  { id:"experience", icon:"✏️", title:"경험을 나타내는 글", color:"#9C6FE4", fruit:"🌸",
    ...
    criteria:[
      {id:"c1",icon:"📌",label:"경험이 잘 드러났나요?",desc:"쓰고 싶었던 경험이 글에 잘 나타났나요?",
        examples:{"○":"언제, 어디서, 무슨 일이 있었는지 상황이 구체적으로 드러남",
                  "△":"경험을 쓰긴 했지만 상황이 막연하거나 짧게만 나타남",
                  "✕":"어떤 경험을 쓴 것인지 알기 어려움"}},
      {id:"c2",icon:"💝",label:"생각·느낌 표현",       desc:"자신의 생각이나 느낌을 잘 표현했나요?",
        examples:{"○":"그때의 생각이나 느낌을 이유와 함께 자세히 표현함",
                  "△":"'기쁘다', '슬프다'처럼 감정 단어만 간단히 씀",
                  "✕":"생각이나 느낌이 전혀 표현되지 않음"}},
      {id:"c3",icon:"🎨",label:"감각적 표현",           desc:"감각적 표현을 사용해 실감나게 표현했나요?",
        examples:{"○":"보고, 듣고, 맡고, 만지는 느낌이 생생하게 표현됨 (예: 차가운 바람이 뺨을 스쳤다)",
                  "△":"감각적 표현이 한두 군데 있지만 충분하지 않음",
                  "✕":"감각적 표현이 전혀 없이 사실만 나열함"}},
    ]},
];
...
export default function App(){
      ...
      <div style={wrap}><div style={mw}>
        ...
        <Card style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:10}}>
            {["○","△","✕"].map(m=>{
              const sel=marks[cur.id]===m;
              const cfg=MARK[m];
              return(
                <button key={m} onClick={()=>setMarks(p=>({...p,[cur.id]:m}))}
  ...
  if(screen==="result"){
    ...
    if(!last) return null;
    const showRevise=crit.filter(c=>!c.isReflect).some(c=>marks[c.id]==="△"||marks[c.id]==="✕");
    return (
      <div style={wrap}><div style={mw}>
  ...
  if(screen==="revisePrompt"){
    const weak=crit.filter(c=>!c.isReflect&&(origMks[c.id]==="△"||origMks[c.id]==="✕"));
    return (
      <div style={{...wrap,justifyContent:"center"}}><div style={mw}>
        <div style={{textAlign:"center",marginBottom:20}}>
        ...
        <Card style={{marginBottom:12,background:"#FFF9E6",border:"1.5px solid rgba(245,185,66,0.4)"}}>
          ...
          {weak.map(c=>(
            <div key={c.id} style={{display:"flex",alignItems:"center",gap:8,
              ...
              <span style={{fontSize:14,fontWeight:700,color:origMks[c.id]==="△"?C.amber:C.red}}>
                {origMks[c.id]==="△"?"잘함":"보통"}
              </span>
            </div>
          ))}
